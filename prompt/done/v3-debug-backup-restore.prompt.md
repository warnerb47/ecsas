# Prompt 1
I am using angular with tauri v2 and sqlite. Here is my backup restore command:
`
use sqlx::SqlitePool;
...
`
ERROR error returned from database: (code: 1) no such table: backup_db.core_user
`
Seem like the ATTACH DATABASE request is not working. How can I fix it ?

# Prompt 2
For debuging I use this request `SELECT seq, name, file FROM pragma_database_list` but here is the result: `ID: 0 | Name: 'main' | File: C:\Users\SNGAYEM\AppData\Roaming\ecsas\ecsas.db`. How Can I fix the backup attach.
Here is the updated merge_databases method
`
pub async fn merge_databases(target_path: &PathBuf, source_path: &PathBuf) -> Result<(), String> {
    // Create connection pool to target database
    let pool = SqlitePool::connect(&format!("sqlite:{}", target_path.display()))
    ...
    let results: Vec<(i64, String, String)> =
        sqlx::query_as("SELECT seq, name, file FROM pragma_database_list")
            .fetch_all(&pool) // Use fetch_all to get all attached databases
            .await
